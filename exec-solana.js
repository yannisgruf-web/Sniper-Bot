...
function init() {
  ...
  try {
    ...
    if (secret.length !== 64 && secret.length !== 32)
      throw new Error(`Key hat ${secret.length} Byte (erwartet 64). Phantom -> Export Private Key (Base58).`);
    wallet = secret.length === 64 ? Keypair.fromSecretKey(secret) : Keypair.fromSeed(secret);
