...
// Momentum-Ausbruch: starke Kerze (großer Kurssprung) + Volumen-Spike.
// +1 = bullischer Ausbruch (long), -1 = bärischer Ausbruch (short), 0 = keiner.
// Für die Momentum-Strategie in Trendphasen (hoher ADX).
function momentumBreakout(candles, opts = {}) {
  const bodyMinPct = opts.bodyMinPct ?? 2.0;   // Mindest-Kerzenkörper in % (Kurssprung)
  const volMult = opts.volMult ?? 2.0;         // Volumen muss X-fachen des Schnitts erreichen
  const volLen = opts.volLen ?? 20;
  const n = candles.length;
  if (n < volLen + 2) return { dir: 0 };
  const k = candles[n - 1];                     // letzte abgeschlossene Kerze
  // Körpergröße relativ zum Eröffnungspreis (Richtung = Vorzeichen).
  const bodyPct = ((k.c - k.o) / k.o) * 100;
  // Durchschnittsvolumen der vorangehenden volLen Kerzen (ohne die aktuelle).
  const vols = candles.slice(n - 1 - volLen, n - 1).map(c => c.v);
  const avgVol = vols.reduce((a, b) => a + b, 0) / vols.length;
  const volSpike = avgVol > 0 && k.v >= avgVol * volMult;
  if (!volSpike) return { dir: 0, bodyPct: +bodyPct.toFixed(2), volRatio: avgVol > 0 ? +(k.v / avgVol).toFixed(1) : 0 };
  let dir = 0;
  if (bodyPct >= bodyMinPct) dir = 1;           // starke grüne Kerze + Volumen -> bullisch
  else if (bodyPct <= -bodyMinPct) dir = -1;    // starke rote Kerze + Volumen -> bärisch
