...
function openPosition(p) {
  ...
  open.set(p.id, pos);
  stats.opened++;
  if (cfg.LOG_PAPER) console.log(`[PAPER OPEN] ${p.symbol} (${p.source}) @ ${p.entryPrice} ${p.priceUnit}`);
  return pos;
}
...
function closePosition(id, exitPrice, reason, onClose) {
  ...
  if (cfg.LOG_PAPER) console.log(`[PAPER CLOSE] ${trade.symbol} ${trade.pnlPct}% (${reason}) nach ${trade.holdMin} Min`);
  onClose?.(trade);
}
