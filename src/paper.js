...
const byReason = {}; // exit-grund -> anzahl
...
function closePosition(id, exitPrice, reason, onClose) {
  ...
  if (!p) return;
  open.delete(id);
  let pnlPct = (exitPrice - p.entryPrice) / p.entryPrice * 100;
  ...
  const rKey = reason.replace(/ \(.*\)$/, "").split(" (")[0];
  byReason[rKey] = (byReason[rKey] || 0) + 1;
