...
const TRADES_KEY = "sniper-trades";
...
function openPosition(p) {
  ...
  const pos = { ...p, openedAt: Date.now(), high: p.entryPrice, sizeUsd: cfg.POSITION_USD };
  open.set(p.id, pos);
  stats.opened++;
...
// Persistenz: alle 2 Min gesammelte Trades in Blobs schreiben (überlebt Neustarts)
async function flush() {
  if (!closedBuffer.length) return;
  const buf = closedBuffer; closedBuffer = [];
  const prev = await getJSON(TRADES_KEY, { trades: [] });
  prev.trades = [...(prev.trades || []), ...buf].slice(-500);
  prev.updated = new Date().toISOString();
  await setJSON(TRADES_KEY, prev);
}
...
function summary() {
  return {
    ...
    openPositions: [...open.values()].map(p => ({
      ...
      pnlNow: null, ageMin: +((Date.now() - p.openedAt) / 60e3).toFixed(1)
    }))
  };
}

module.exports = { openPosition, updatePrice, closePosition, canOpen, summary, stats, open, flush };
