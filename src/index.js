...
async function bnbUsd() {
  ...
  try {
    ...
    if (d && d.price) { _bnb = { p: +d.price, t: Date.now() }; return _bnb.p; }
  } catch (e) { console.error("bnbUsd fetch fehlgeschlagen:", e.message); }
  // Fallback: letzter bekannter Wert, sonst ein grober Näherungswert (NIE 0 -> sonst zeigt Guthaben fälschlich $0)
  return _bnb.p || 620;
}
