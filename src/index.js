  ...
  try {
    ...
    for (const u of (d.result || [])) {
      ...
      if (text === "/stop") { risk.kill("manuell"); notify("🛑 <b>NOT-AUS aktiv</b>\nKeine neuen Live-Käufe mehr. Offene Positionen werden weiter regulär verkauft. Mit /go wieder freigeben.").catch(()=>{}); }
        ...
        notify("📈 Berechne aktuellen Stand...").catch(()=>{});
        const s = await indi.statusLive();
        notify(`📈 <b>Indikator-Engine (Paper)</b>\nGesamtwert: ${s.gesamt.toFixed(2)}$ · frei: ${s.balance.toFixed(2)}$\n${s.offen.length ? `Offener PnL: ${s.offenerPnl >= 0 ? "+" : ""}${s.offenerPnl.toFixed(2)}$\n\nOffene Positionen (${s.offen.length}):\n` + s.offen.join("\n") : "Keine offenen Positionen"}`).catch(()=>{});
      }
        ...
        const pfeil = r.renditePct >= 0 ? "📈" : "📉";
        const vz = r.renditePct >= 0 ? "+" : "";
        notify(`${pfeil} <b>Gesamt-Rendite: ${vz}${r.renditePct.toFixed(1)}%</b>\n\n` +
          `Start: ${r.start.toFixed(2)}$\nJetzt: ${r.jetzt.toFixed(2)}$\n` +
          `${vz}${r.gewinnUsd.toFixed(2)}$ seit Beginn\n\n` +
          `Davon realisiert: ${r.realisiertUsd >= 0 ? "+" : ""}${r.realisiertUsd.toFixed(2)}$ (${r.tradeAnzahl} Trades)\n` +
          `Offen (${r.offenAnzahl}): ${r.offenerPnl >= 0 ? "+" : ""}${r.offenerPnl.toFixed(2)}$ unrealisiert\n` +
          `Frei: ${r.frei.toFixed(2)}$`).catch(()=>{});
      }
        ...
        if (!b.n) { notify("📈 Noch keine abgeschlossenen Indikator-Trades.").catch(()=>{}); }
          ...
          notify(`📈 <b>Indikator-Bilanz</b> — ${b.n} Trades gesamt (nach Gebühren)` +
            ...
            block("Long", b.long) +
            block("Short", b.short) +
            (b.oszillator ? block("↔️ Oszillator", b.oszillator) : "") +
            (b.momentum ? block("⚡ Momentum", b.momentum) : "") +
