...
const checks = {
  ...
  gesperrt_creator: 0, exit_wash: 0, exit_insider: 0
};
...
function decide(mint) {
  ...
  if (buys.length < 2) { pump.unsubscribeTrades(mint); paper.stats.noEntry = (paper.stats.noEntry || 0) + 1; return; }
  ...
  if (!pos) { pump.unsubscribeTrades(mint); return; }
...
setInterval(async () => {
  for (const [id, ref] of gtOpen) {
    ...
    if (price) paper.updatePrice(id, price, trade => { gtOpen.delete(id); notify(tradeExitMsg(trade)).catch(() => {}); });
  }
