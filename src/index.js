...
startGeckoListener(async cand => {
  ...
  if (cand.priceUsd && cand.poolAddr) {
    ...
    setTimeout(async () => {
      ...
      if (delay > 0) { const fresh = await poolPriceUsd(cand.network, cand.poolAddr); if (fresh) fill = fresh; }
      const pos = paper.openPosition({ id: cand.network + ":" + cand.address, symbol: cand.symbol, source: "gecko", network: cand.network, entryPrice: fill, priceUnit: "USD", link });
