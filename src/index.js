...
let bnbUsdCache = { v: 0, t: 0 };
async function bnbUsd() {
  if (Date.now() - bnbUsdCache.t < 60e3 && bnbUsdCache.v) return bnbUsdCache.v;
  try { const r = await fetch("https://api.binance.com/api/v3/ticker/price?symbol=BNBUSDT"); const d = await r.json(); if (d.price) bnbUsdCache = { v: +d.price, t: Date.now() }; } catch {}
  return bnbUsdCache.v || 620;
}
const boot = Date.now();
  ...
  if (cand.priceUsd && cand.poolAddr) {
    ...
    setTimeout(async () => {
      ...
      if (pos) {
        ...
        if (executor.enabled(cand.network)) { const prices = { sol: await solUsd(), bnb: await bnbUsd() }; executor.onEntry(posId, cand.network, cand.address, prices).catch(e => console.error("exec.onEntry", e.message)); }
      }
    }, delay);
  }
});
...
setInterval(async () => {
  ...
  for (const [net, list] of Object.entries(byNet)) {
    ...
    for (const x of list) {
      ...
      if (price) paper.updatePrice(x.id, price, trade => { gtOpen.delete(x.id); executor.onExit(x.id, trade.pnlPct).catch(()=>{}); notify(tradeExitMsg(trade)).catch(() => {}); });
    }
  }
...
async function pollCommands() {
  ...
  try {
    ...
    for (const u of (d.result || [])) {
      ...
      else if (text === "/status") { const s = executor.summary(); notify(`📊 Live: ${s.aktiv?"an":"aus"} · offen ${s.offeneLivePositionen}\nKäufe ${s.buys} (Fails ${s.buyFails}) · Verkäufe ${s.sells} (Fails ${s.sellFails})\nRealisiert $${s.realizedUsd} · heute $${s.risk.realizedUsdToday}\nKill: ${s.risk.killed?s.risk.killReason:"nein"}`).catch(()=>{}); }
    }
  } catch {}
...
(async () => {
  await wallets.initSolana();
  await wallets.initBsc();
  if (cfg.LIVE_TRADING) {
    const sBal = await wallets.solBalanceUsd(await solUsd()).catch(()=>null);
    const bBal = await wallets.bnbBalanceUsd(await bnbUsd()).catch(()=>null);
    ...
    log(`   Positionsgröße SOL $${cfg.POSITION_USD_SOLANA} / BSC $${cfg.POSITION_USD_BSC} | Tageslimit -$${Math.abs(cfg.MAX_DAILY_LOSS_USD)}`);
    notify(`⚠️ <b>LIVE-TRADING gestartet</b>\nChains: ${cfg.LIVE_CHAINS.join(", ")}\nSolana: ${sBal!=null?"$"+sBal.toFixed(2):"—"} · BSC: ${bBal!=null?"$"+bBal.toFixed(2):"—"}\nPos: SOL $${cfg.POSITION_USD_SOLANA} / BSC $${cfg.POSITION_USD_BSC} · Tageslimit -$${Math.abs(cfg.MAX_DAILY_LOSS_USD)}`).catch(()=>{});
  } else {
