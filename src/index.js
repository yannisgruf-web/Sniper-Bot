...
let _bnb = { p: 0, t: 0 };
async function bnbUsd() {
  if (Date.now() - _bnb.t < 60e3 && _bnb.p) return _bnb.p;
  try { const r = await fetch("https://api.binance.com/api/v3/ticker/price?symbol=BNBUSDT"); const d = await r.json(); if (d.price) { _bnb = { p: +d.price, t: Date.now() }; } } catch {}
  return _bnb.p || 0;
}
