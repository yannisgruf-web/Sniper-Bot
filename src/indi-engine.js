...
function closePos(sym, price, grund) {
  ...
  persist();
  logTrade({ symbol: sym.replace("USDT", ""), side, strategie: pos.strategie || "oszillator", hebel, entry: pos.entry, exit: price, sizeUsd: pos.usd,
    ...
    for (const sym of await topSymbols()) {
      try {
        ...
        if (!pos) {
          ...
          // ── ADX-Weiche: entscheidet, welche Strategie für diesen Coin zuständig ist.
          // Hoher ADX = starker Trend -> Momentum. Niedriger ADX = ruhig -> Oszillatoren.
          const ax = cfg.INDI_MOMENTUM ? ind.adx(candles) : null;
          const trendPhase = ax != null && ax >= cfg.INDI_MOMENTUM_ADX;
          ...
          // Bestehende Position: schließen bei Gegen-Konfluenz (nur bei Oszillator-Trades;
          // Momentum-Trades werden ausschließlich über TP/SL/Trailing beendet).
          if ((pos.strategie || "oszillator") === "oszillator") {
...
// Gesamtrendite seit Start: aktueller Gesamtwert (frei + offene Positionen inkl.
// Live-PnL) gegen das ursprüngliche Startkapital. Eine einzige Zahl fürs große Bild.
async function gesamtrendite() {
  const s = await statusLive();                 // liefert gesamt inkl. offener Live-PnL
  const start = state.startkapital || cfg.INDI_START_USD;
  const jetzt = s.gesamt;
  const gewinnUsd = jetzt - start;
  const renditePct = start > 0 ? (gewinnUsd / start) * 100 : 0;
  // Zusätzlich: realisierter Anteil aus abgeschlossenen Trades.
  const trades = load(TRADES, []);
  const realisiert = trades.reduce((a, t) => a + (t.pnlUsd || 0), 0);
  return {
    start, jetzt, gewinnUsd, renditePct,
    frei: state.balance, offenAnzahl: Object.keys(state.positions).length,
    offenerPnl: s.offenerPnl, realisiertUsd: realisiert, tradeAnzahl: trades.length,
  };
}
...
function bilanz() {
  ...
  const oszi = arr.filter(t => (t.strategie || "oszillator") === "oszillator");
  const mom = arr.filter(t => t.strategie === "momentum");
  return { n: arr.length, gesamt: auswert(arr), long: auswert(longs), short: auswert(shorts),
           oszillator: auswert(oszi), momentum: auswert(mom) };
}
