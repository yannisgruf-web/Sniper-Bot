...
function recordRealized(usd) { rollDay(); realizedUsdToday += usd; if (realizedUsdToday <= -Math.abs(cfg.MAX_DAILY_LOSS_USD)) { killed = true; killReason = "tageslimit"; } }
...
function status() { rollDay(); return { killed, killReason, realizedUsdToday: +realizedUsdToday.toFixed(2), limit: -Math.abs(cfg.MAX_DAILY_LOSS_USD) }; }
